...
# Serves repeatedly rather than once: a test suite opens a session per case, and
# a host that accepts one connection would make the second case look like a
# protocol failure.
while True:
    c, _ = s.accept()
    try:
        serve(c)
    except Exception as error:  # noqa: BLE001 - a dropped client must not end the host
        print("client ended: %r" % (error,), file=sys.stderr)
    finally:
        try:
            c.close()
        except OSError:
